...
function Productsales({ prod }: ProductsProps) {
    ...
    const removeFromWishlist = () => {
        axios.delete(`http://localhost:8080/fav/deletehome/1/${prod.prodId}`) // Assuming user ID is 1
            .then((res) => {
    ...
    return (
        <div className="w-full h-2/6">
            <div className="relative">
                ...
                <div className="absolute top-5 right-4 p-2" onClick={fav ? removeFromWishlist : addToWishlist}>
                    {fav ? <MdFavorite style={{ color: "#17998a" }} className='w-8 h-8' /> : <GrFavorite className='w-8 h-8' />}
                </div>
            </div>
